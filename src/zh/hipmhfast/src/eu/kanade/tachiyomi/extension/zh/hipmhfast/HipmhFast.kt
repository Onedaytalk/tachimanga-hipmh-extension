...
@Source
abstract class HipmhFast : KeiSource() {
    ...
    override suspend fun fetchMangaUpdate(
        ...
        val mangaDeferred = if (fetchDetails) {
            async {
                fetchMangaSummaryOrNull(manga)
                    ?.toSManga(::resolveCoverUrl)
                    ?.apply { url = manga.url }
        ...
        val chaptersDeferred = if (fetchChapters) {
            async { fetchChapterList(manga, chapters) }
        } else {
            null
        }
    ...
    private suspend fun fetchMangaSummaryOrNull(manga: SManga): MangaDetailsData? = try {
        val url = "$apiUrl/v1/manga".toHttpUrl().newBuilder()
            .addQueryParameter("mid", manga.mangaId())
            .build()
        val response = client.get(url).parseAs<ApiResponse<MangaDetailsData>>()
        response.requireSuccess()
        response.data
    ...
    private suspend fun fetchChapterList(manga: SManga, existingChapters: List<SChapter>): List<SChapter> = coroutineScope {
        val mid = manga.mangaId()
        val firstPage = fetchChapterPage(mid, 1)
        val firstPageChapters = firstPage.items.map(ChapterItem::toSChapter)

        if (existingChapters.isNotEmpty() && firstPage.total != null) {
            val existingIds = existingChapters.asSequence().map { it.normalizedId() }.toSet()
            val hasOverlap = firstPageChapters.any { it.normalizedId() in existingIds }
            val merged = (firstPageChapters + existingChapters).distinctBy { it.normalizedId() }
            if (hasOverlap && merged.size == firstPage.total) {
                return@coroutineScope merged
            }
        }

        val remainingPages = (2..firstPage.totalPages).map { page ->
            async { fetchChapterPage(mid, page).items }
        }.awaitAll()

        firstPageChapters + remainingPages.flatten().map(ChapterItem::toSChapter)
    }

    private suspend fun fetchChapterPage(mid: String, page: Int): ChapterPageData {
    ...
    private fun SManga.mangaId(): String = url.substringAfter("/works/").substringBefore('-')
        .takeIf(String::isNotBlank)
        ?: error("Missing manga id")

    private fun SChapter.normalizedId(): String = url.substringBefore('?').substringAfterLast('/')
